  ...
  if (message.isSystem) {
    return (
      <div className="system-message">
        {message.systemIcon === 'pin' && (
          <svg className="system-message-icon" width="12" height="12" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            ...
            <div className="message-cards">
              {message.cards.map((card, i) => card.type === 'file' ? (
                ...
                <div key={i} className="adaptive-card" style={{ borderLeftColor: card.accentColor }}>
                        ...
                        <div key={j} className="card-section">
                          ...
                          {section.diff && (
                            <div className="card-diff">
                              {section.diff.map((line, k) => (
                                <div key={k} className={`card-diff-line card-diff-line-${line.type === '+' ? 'add' : line.type === '-' ? 'remove' : 'context'}`}>
                                  <span className="card-diff-gutter">{line.type}</span>
                                  <span className="card-diff-text">{line.text}</span>
                                </div>
                              ))}
                            </div>
                          )}
                  ...
                  {/* Action row. Actions can be strings or objects { label, type, primary }. */}
                  {card.actions && (
